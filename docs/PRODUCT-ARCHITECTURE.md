...
The Tenant Model Execution Bridge can invoke explicitly approved model roles through customer-owned Agents. The server derives a binding from the exact task, node, model, connection configuration digest and epoch; the Agent rechecks it against private local configuration. The current concrete Adapter supports loopback Ollama and returns bounded result/evidence digests plus observed input/output tokens. Tenant Agents sharing a customer-managed key can relay AES-256-GCM encrypted role artifacts through the server without exposing plaintext or the key. It does not authorize commands, repository writes, Git publication, database mutation, deployment, or automatic recovery. Transactional multi-instance persistence, object storage and retention, customer KMS integration, additional Provider Adapters and an external evidence-verification service remain subsequent work.
