...
import { configureOllama, enroll, executeDispatch, executeNextDispatch, heartbeat } from '../agent/node-agent.mjs';
...
test('execution receipt binds exact claim, provider result and observed token usage',()=>{
  const binding=buildExecutionBindings({task,models,connections})[0],receipt=executionReceiptContract({schema_version:1,
    ...
    activation_receipt:H('activation'),binding,prompt_digest:H('prompt'),response_digest:H('response'),model_revision:H('model-revision'),status:'SUCCEEDED',failure_reason:null,
...
test('approved Head dispatch executes on the customer Agent and returns bounded usage evidence',async()=>{
  const priorAdapters=process.env.EODUKSINI_AGENT_ADAPTERS;process.env.EODUKSINI_AGENT_ADAPTERS='ollama';
  ...
  await store.upsertIdentity({github_id:'42',login:'owner',avatar_url:null});const organization=store.organizationsForUser('42')[0],organization_id=organization.organization_id,
    ...
    model=(await store.createModel({organization_id,connection_id:connection.connection_id,provider_model_id:'fixture-model',display_name:'Fixture model',
      role_capabilities:['head','planner']})).model,entitlement=subscriptionEntitlement({status:'active',plan_id:'pro'}),
    ...
    control=createStudioServer({auth,store}),provider=createServer(async(request,response)=>{if(request.url==='/api/tags'){
      response.writeHead(200,{'content-type':'application/json'});response.end(JSON.stringify({models:[{name:'fixture-model',digest:H('fixture-model-revision')}]}));return}
      let body='';for await(const chunk of request)body+=chunk;const payload=JSON.parse(body);assert.equal(request.url,'/api/generate');
      assert.equal(payload.model,'fixture-model');assert.match(payload.prompt,/TASK-EXEC-1/);
      response.writeHead(200,{'content-type':'application/json'});response.end(JSON.stringify({model:'fixture-model',response:'HEAD_RESULT',prompt_eval_count:21,eval_count:4}))});
  ...
  const controlOrigin=`http://127.0.0.1:${control.address().port}`,providerOrigin=`http://127.0.0.1:${provider.address().port}`,agentRoot=join(parent,'agent'),
  ...
  try{const node=await enroll(controlOrigin,agentRoot,'enr_missing').catch(()=>null);assert.equal(node,null);
    ...
    const profile=await store.saveOrchestrationProfile({organization_id,mode:'automatic',
      head_model_id:model.model_id,assignments:{planner:null,coder:null,reviewer:null,validator:null}}),created=await post('/api/organization/tasks',{
    ...
    const executed=await executeNextDispatch(agentRoot,'exec-1');assert.equal(executed.status,'SUCCEEDED');assert.equal(executed.response_text,'HEAD_RESULT');
    const [current]=await store.dispatchTasks(organization_id);assert.equal(current.dispatches[0].status,'SUCCEEDED');assert.equal(current.dispatches[1].status,'QUEUED');
    ...
    assert.equal(current.attempts[0].execution_receipt.model_revision,H('fixture-model-revision'));
