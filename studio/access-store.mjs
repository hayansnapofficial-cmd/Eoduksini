...
const empty=()=>({schema_version:2,users:{},processed_webhook_ids:[],billing_requests:{}});
...
function migrate(data) {
  ...
  return {schema_version:2,users,processed_webhook_ids:data.processed_webhook_ids??[],billing_requests:{}};
}

function validate(input) {
  const data=migrate(input);
  check(data&&Object.keys(data).length===4&&data.schema_version===2&&data.users&&typeof data.users==='object'&&!Array.isArray(data.users)&&
  ...
  for(const [id,user] of Object.entries(data.users))check(validId(id)&&user&&Object.keys(user).length===9&&user.github_id===id&&safeText(user.login,128)&&
  ...
  'INVALID_ACCESS_STORE');
  for(const [id,request] of Object.entries(data.billing_requests))check(safeText(id,128)&&request&&Object.keys(request).length===8&&
    ...
    request.expected_price>=0&&nullableText(request.subscription_id,128)&&['pending','active','past_due','canceled'].includes(request.status)&&
...
export function createAccessStore(root) {
  ...
  if(raw().schema_version===1)save(migrate(raw()));
  ...
  return {
    ...
    createBillingRequest(request){return update(data=>{check(safeText(request.request_id,128)&&validId(String(request.github_id))&&
      PROVIDERS.has(request.provider)&&Number.isSafeInteger(request.expected_price)&&request.expected_price>=0&&data.users[String(request.github_id)]&&
      ...
      data.billing_requests[request.request_id]={request_id:request.request_id,github_id:String(request.github_id),provider:request.provider,
        expected_price:request.expected_price,subscription_id:null,status:'pending',created_at:now,updated_at:now};
    ...
    applySubscription({event_id,provider,github_id,customer_id,subscription_id,status,current_period_end,request_id=null}){return update(data=>{
      ...
      if(request_id!==null){const request=data.billing_requests[request_id];check(request&&request.github_id===String(github_id)&&request.provider===provider&&
        (request.subscription_id===null||request.subscription_id===subscription_id),'INVALID_BILLING_REQUEST');request.subscription_id=subscription_id;
      ...
      user.subscription_status=status;user.current_period_end=current_period_end;user.updated_at=new Date().toISOString();
