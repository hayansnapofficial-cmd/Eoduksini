...
const RECEIPT_KEYS=['schema_version','organization_id','task_id','dispatch_id','attempt_id','dispatch_epoch','activation_receipt','binding','prompt_digest',
  'response_digest','model_revision','artifact_id','status','failure_reason','input_tokens','output_tokens','usage_status','started_at','finished_at'];
...
export function executionPromptWithArtifact(envelope,text) {const base=executionPrompt(envelope);if(envelope.predecessor_result_digest===null){
    check(text===null,'UNEXPECTED_PREDECESSOR_ARTIFACT');return base}check(typeof text==='string'&&text.isWellFormed()&&Buffer.byteLength(text,'utf8')<=49_152&&
    createHash('sha256').update(text).digest('hex')===envelope.predecessor_result_digest,'PREDECESSOR_ARTIFACT_MISMATCH');return `${base}\nPredecessor artifact:\n${text}`}

const receiptEvidence=value=>digest(Object.fromEntries(RECEIPT_KEYS.filter(key=>!(key==='artifact_id'&&value[key]===null)).map(key=>[key,value[key]])));
...
export function executionReceiptContract(input) {
  ...
  check(value.schema_version===1&&safe(value.organization_id,64)&&safe(value.task_id,128)&&safe(value.dispatch_id,256)&&safe(value.attempt_id,128)&&
    ...
    HASH.test(value.response_digest)&&(value.model_revision===null||safe(value.model_revision,256))&&
    (value.artifact_id===null||/^artifact-[0-9a-f]{64}$/.test(value.artifact_id))&&['SUCCEEDED','FAILED'].includes(value.status)&&
