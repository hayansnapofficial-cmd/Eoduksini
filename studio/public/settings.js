...
async function load(){try{const [session,catalogData]=await Promise.all([fetch('/api/session').then(value=>value.json()),fetch('/api/provider-catalog').then(value=>value.json())]);
  ...
  await Promise.all([reload(catalog),reloadNodes()]);$('provider-form').addEventListener('submit',async event=>{event.preventDefault();try{await post('/api/organization/providers',{provider_id:$('provider').value,display_name:$('connection-name').value.trim()});
  ...
  $('copy-token').addEventListener('click',async()=>{try{await navigator.clipboard.writeText($('enrollment-token').textContent);$('node-message').textContent='토큰을 복사했습니다.'}
    catch{$('node-message').textContent='복사할 수 없습니다. 토큰을 직접 선택해 주세요.'}});setInterval(()=>reloadNodes().catch(()=>{}),30_000)
